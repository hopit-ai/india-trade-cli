...
def analyse(
    ...
    if rsi_val < 30:
        ...
        signals.append(Signal("RSI", round(rsi_val, 1), "NEUTRAL",
    ...
    # Bollinger Bands
    if ltp < bb_l:
        signals.append(Signal("BollingerBands", round(ltp, 2), "BULLISH", "Price below lower band — potential reversal"))
        score += 10
    elif ltp > bb_u:
        signals.append(Signal("BollingerBands", round(ltp, 2), "BEARISH", "Price above upper band — potential reversal"))
        score -= 10
    else:
        bb_pos = (ltp - bb_l) / (bb_u - bb_l) if (bb_u - bb_l) else 0.5
        signals.append(Signal("BollingerBands", f"{bb_pos:.0%} of band", "NEUTRAL",
                              f"Upper={bb_u:.2f} Mid={bb_m:.2f} Lower={bb_l:.2f}"))

    # Volume confirmation
    if vol_ratio > 1.5:
        dir_signal = "BULLISH" if ltp >= float(df["open"].iloc[-1]) else "BEARISH"
        signals.append(Signal("Volume", f"{vol_ratio:.1f}x avg", dir_signal,
                              f"High volume day ({vol_ratio:.1f}× 20-day avg)"))
        score += 8 if dir_signal == "BULLISH" else -8
    else:
        signals.append(Signal("Volume", f"{vol_ratio:.1f}x avg", "NEUTRAL",
                              "Normal volume"))
