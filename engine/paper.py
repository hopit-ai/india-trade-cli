...
class PaperBroker(BrokerAPI):
    ...
    def _fill_price(req: OrderRequest, ltp: float) -> float | None:
        ...
        if ot == "MARKET":
            ...
            p = req.price or ltp
            if side == "BUY"  and ltp <= p: return round(ltp, 2)
            if side == "SELL" and ltp >= p: return round(ltp, 2)
    ...
    def _apply(self, req: OrderRequest, fill_price: float) -> None:
        ...
        margin = value if prod == "CNC" else (
            value * MIS_MARGIN if prod == "MIS" else value * NRML_MARGIN
        )
        side = req.transaction_type.upper()
        ...
        if side == "BUY":
            if self._state["cash"] < margin:
                raise ValueError(
                    f"Insufficient funds: need ₹{margin:,.0f}, "
                    f"have ₹{self._state['cash']:,.0f}"
                )
            self._state["cash"] -= margin
            bucket = "holdings" if prod == "CNC" else "positions"
            entry  = self._state[bucket].get(sym, {"qty": 0, "avg_price": 0.0, "product": prod, "exchange": req.exchange})
    ...
    def _record_order(
        self, order_id: str, req: OrderRequest,
        fill_price: float, status: str, message: str = "",
    ) -> None:
